...
def seabed_security(
    ...
    def fitness(weight, weight_file="cache/weight.txt"):
        ...
        fitness = (
            0.5 * np.min(score1_list) - 0.5 * np.min(score2_list)
            if difference_mode
        ...
        if verbose:
            print(
                f"agent's score: {score1_list},\topponent's score: {score2_list},\tfitness: {fitness}"
            )

        return (
            fitness
            if not parallel
            else (int(weight_file.split("_")[-1].split(".")[0]), fitness)
        )

    def parallel_fitness(weights, max_workers=8):
        scores = {}
        weight_files = [
            f"cache/multithread_weight_{num}.txt" for num in range(len(weights))
        ]

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            tasks = {
                executor.submit(fitness, weight, weight_file): (weight, weight_file)
